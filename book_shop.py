...
def get_one_to_many(shops, books):
    return [(b.title, b.price, s.name)
            for s in shops
            for b in books
            if b.shop_id == s.id]

def get_many_to_many(shops, books, book_shop_links):
    many_to_many_temp = [(s.name, bs.shop_id, bs.book_id)
                         for s in shops
                         for bs in book_shop_links
                         if s.id == bs.shop_id]

    return [(b.title, b.price, shop_name)
...
def task_a3(shops, many_to_many):
    print('\nЗадание A3')
...
def main():
    ...
    many_to_many = get_many_to_many(shops, books, book_shop_links)

    task_a1(one_to_many)
    task_a2(shops, one_to_many)
    task_a3(shops, many_to_many)
